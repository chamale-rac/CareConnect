...
function Stock() {
	const [id_instalacion_medica, setid_instalacion_medica] = useState('1')
	...
	const handleSearch = async () => {
		try {
			...
			const filteredData = data.filter((producto) => {
				...
				if (
					startDate &&
					new Date(producto.fecha_exp) < new Date(startDate)
				) {
					return false
				}
				if (
					endDate &&
					new Date(producto.fecha_exp) > new Date(endDate)
				) {
					return false
				}
	...
	return (
		<Container className="glossy-card-dark mt-5 p-5">
			<Row>
				<Col>
